...
class NestedSetsTreeBehavior extends \yii\base\Behavior {
    ...
    public function treeCategories($controller) {
        $makeNode = function ($node, $controller) {
            $newData = [
                ...
                $this->hrefOutAttribute => ['/'.$controller.'/default/category', 'categoryalias' => $node['alias']],
            ];
    ...
    public function listTreeCategories($controller) {
        ...
        // get cache
        $trees = Yii::$app->cache->get('listTreeCategories' . $controller . Yii::$app->language);
        if (empty($trees)) {
            // Trees mapped
    ...
    public function listAll() {
        $makeNode = function ($node) {
            $newData = [
                $this->labelOutAttribute => ($node[$this->depthAttribute] > 0) ? str_pad('', ($node[$this->depthAttribute]), '-') . ' ' . $node['translate'][$this->labelAttribute] : $node['translate'][$this->labelAttribute],
            ];
            return array_merge($node, $newData);
        };
        if (empty($trees)) {
            // Trees mapped
            $trees = $this->owner->find()->with('translate')->orderBy(['tree' => SORT_ASC, 'lft' => SORT_ASC])->asArray()->all();
            if (count($trees) > 0) {
                foreach ($trees as &$col) $col = $makeNode($col);
            }
        }
