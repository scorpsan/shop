...
class CategoriesController extends \backend\controllers\AppController {
    ...
    public function actionCreate() {
        ...
        $root = Categories::find()->where(['alias' => 'shop'])->limit(1)->one();
        $parentList = ArrayHelper::map($root->listTreeCategories('shop', $this->clearRoot), 'id', 'title');
    ...
    public function actionDelete($id) {
        if (($model = Categories::findOne($id)) !== null) {
            try {
                ...
                //ShopProducts::updateAll(['category_id' => 0], ['category_id' => $id]);
                $model->delete();
                $this->clearCache();
    ...
    private function clearCache() {
        ...
        foreach ($languages as $language) {
            Yii::$app->cache->delete('basicTreeCategories' . 'shop' . $language->url);
            Yii::$app->cache->delete('listTreeCategories' . 'shop' . $language->url);
            Yii::$app->cache->delete('menuTreeCategories' . 'shop' . $language->url);
        }
    }
